...
type ProductLandingProps = { eyebrow: string; title: string; accent: string; description: string; outcome: string; steps: [string, string, string]; cta: string; ctaHref?: string }
const stepIcons = [Sparkles, MessageCircleMore, CalendarCheck]

export default function ProductLanding({ eyebrow, title, accent, description, outcome, steps, cta, ctaHref = "/contact" }: ProductLandingProps) {
  return <main className="growth-page growth-product-page">
    <section className="growth-product-hero"><div className="growth-shell"><p className="growth-kicker"><span /> {eyebrow}</p><div className="growth-product-title"><h1>{title} <em>{accent}</em></h1><p>{description}</p></div><Link href={ctaHref} className="growth-button growth-button-dark">{cta} <ArrowUpRight size={17} /></Link></div></section>
    ...
    <section className="growth-product-cta"><div className="growth-shell"><p className="growth-kicker">Ready when you are</p><h2>Make every opportunity count.</h2><Link href={ctaHref} className="growth-button growth-button-light">{cta} <ArrowUpRight size={17} /></Link></div></section>
  </main>
}
